...
fn falling_block_game(gba: &'static mut Gba) -> ! {
    ...
    s.sprites
        .push(Box::new(sprites.get_vram_sprite(melon_sprite)));

    let cross_sprite = MISC_GRAPHICS.sprites().first().unwrap();

    s.sprites
        .push(Box::new(sprites.get_vram_sprite(cross_sprite)));
    ...
    loop {
        ...
        for current_fruit in 0..s.fruits.len() {
            ...
            if fruit.state == FruitState::Held {
                if input.is_pressed(Button::A) {
                    ...
                    fruit.set_position(clamp(
                        fruit.get_position(),
                        fvec(WALL_L as f32 + 8.0, 0.0),
                        fvec(WALL_R as f32 - 8.0, WIDTH as f32),
                    ));
                    continue;
                }
            }
            ...
            let nudge: Vector2D<_> = Default::default();
            ...
            // Collide with other fruits
            {
                ...
                for (n, other_fruit) in rest.into_iter() {
                    ...
                    if intersecting {
                        ...
                        println!(
                            "F1V {:?} F2V {:?}",
                            fruit_physics_object.velocity.0, other_fruit.velocity.0
                        );
                        ...
                        if t <= num!(0.0) {
                            ...
                            let vec_ab =
                                other_fruit.circle().position - fruit_physics_object.position;
            ...
            fruit.set_position(clamp(
                fruit.get_position(),
                fvec(
                    (WALL_L + fruit.radius as i32) as f32,
                    0.0 + fruit.radius as f32,
                ),
                fvec(
                    (WALL_R - fruit.radius as i32) as f32,
                    (HEIGHT - fruit.radius as i32) as f32,
                ),
            ));
            fruit.velocity = fruit_physics_object.velocity;
            fruit.collided_with_fruits = collided_with;

            let moving_left = if fruit.velocity.0.x < num!(0.0) {
                -1
            } else {
                1
            };
            ...
            fruit
                .world_object
                .set_affine_matrix(s.affines[affine_index(fruit.rotation.angle)].clone());
        }
        ...
        if s.fruits
            .get(my_melon)
            .is_some_and(|fruit| fruit.state == FruitState::Rolling)
        {
            ground_timer = 300;
...
fn make_affines() -> [AffineMatrixInstance; 32] {
    ...
    for (n, frac) in fractions.into_iter().enumerate() {
        affines[n] = Some(AffineMatrixInstance::new(
            AffineMatrix::from_rotation(frac.clone().change_base::<_, 4>()).to_object_wrapping(),
        ));
    }
