...
pub static PLAY_AREA: (Wall, Wall, Wall) = (
    Horizontal(WALL_L, WallDirectionHorizontal::Left),
    Horizontal(WALL_R, WallDirectionHorizontal::Right),
    Vertical(FLOOR, WallDirectionVertical::Bottom),
);
...
impl Circle {
    ...
    fn contains(self: Circle, p: Vector2D<Fixed>) -> bool {
        let x = self;
        let distance = (p.change_base() - x.position)
            .change_base::<Fixed>()
            .magnitude_squared();
...
pub fn clamp<T: PartialOrd + Copy + Clone + agb::fixnum::Number>(
    n: Vector2D<T>,
    lower: Vector2D<T>,
    upper: Vector2D<T>,
) -> Vector2D<T> {
...
pub fn two_circle_interpolate(circle_a: MovingCircle, circle_b: MovingCircle) -> Fixed {
    ...
    let distance_squared =
        ((circle_b.before().position) - (circle_a.before().position)).magnitude_squared();
    ...
    let (a, b, c, d) = (
        circle_b.after.1 .0.x,
        circle_a.after.1 .0.x,
        circle_b.after.1 .0.y,
        circle_a.after.1 .0.y,
    );
    let (x1, x2, y1, y2) = (
        circle_a.before.position.x,
        circle_b.before.position.x,
        circle_a.before.position.y,
        circle_b.before.position.y,
    );
    ...
    let quadr_c = sq(x1) + sq(x2) + sq(y1) + sq(y2)
        - (two * ((x1 * x2) + (y1 * y2)))
        - sq(Fixed::from(circle_a.before.radius + circle_b.before.radius));
...
impl Ball {
    ...
    pub fn time_to_collision(&self, other: &Ball) -> Num<i32, 8> {
        ...
        let a =
            (xvel - other.xvel) * (xvel - other.xvel) + (yvel - other.yvel) * (yvel - other.yvel);
        let b =
            num!(2.) * ((x - other.x) * (xvel - other.xvel) + (y - other.y) * (yvel - other.yvel));
